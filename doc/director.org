#+TITLE: [[http://thespianpy.com][@@html:<img src="thesplogo2.png" alt="Thespian" width="50%" style="inline" />@@]] Thespian Director Guide
...
#+LATEX_HEADER: \docrev{1.00}{2017 Nov 27}
...
As described in the [[using.org][Using Thespian]] document, it is possible to use the
...
The Thespian Director is easily invoked from the commandpline via the
python module specification syntax:

#+BEGIN_EXAMPLE
$ python -m thespian.director [command [args...]]
#+END_EXAMPLE
...
ZIPfile as well.  To specify the list of packages that should be
...
is specified to the "gensrc" command as an additional arguemnt,
...
Validated "/opt/thespian/director/foo-201710261902.tls" loadable with key /opt/theshpian/director/sourcekey_tls.key
...
maintained in a separate file whose value can simply be read and
directly used.  This is done to make it easy to update these
configuration files via automated means: no existing configuraiton
format must be read and partially manipulated: simply re-write an
entire file with the new configuration value.
...
parameter is defined for messages.  This parameter will contain the
address of the Actor which generated the message, or an empty string
if the message did not come from an Actor.
...
The Director can be used to start the local Thespian ActorSytem either
...
~ActorSytstem().loadSource()~ function and represents the hash of the
loaded source.
...
primary key is the name of the group.  The value for that is another
...
** Communication across package boundaries
   ...
   namespace for each loaded source.  This means that Actors within
   the same loaded source will share the same hash and can use
   internally created class objects to communicate, but that they will
   not be able to do this when communicating with Actors in other
   loaded sources (or the external environment).  This means that all
   messages exchanged with Actors across a loaded source boundary must
   be regular, built-in Python objects (strings, numbers, booleans,
   tuples, lists, dictionaries, etc.).

   If there is an attempt to send an internal source-loaded object
   across this boundary, the unpickling error will be written to the
   logile for the destination actor, but it will not even be able to
   generate a PoisonMessage response because it cannot parse the
...
fail2 = ActorSytem().createActor(FooActor)
#+END_EXAMPLE

however, the following will work correctly:

#+BEGIN_EXAMPLE
good_addr = ActorSytem().createActor('foo.fooActor.FooActor', ...)
#+END_EXAMPLE
