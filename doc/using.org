...
#+LATEX_HEADER: \docrev{2.8}{2015 Oct 18}
...
#+end_src
   ...
   to create a [[*Named Actors][Named Actors]] instance.
...
** preRegisterRemoteSystem
   ...
   Actor System about a remote Actor System.

   /This method is normally not used or needed, and should be avoided
   if possible./

   Normally Actor Systems will communicate with each other and
   ...
   method can be used to initiate communications between the two Actor
   Systems.  This method will only work for Actor System bases that
   support Conventions (as describe in the [[*Other Actor Systems][Other Actor Systems]]
   section).

   Because the remote Actor System and the local Actor System cannot
...
#+begin_src python
     self.preRegisterRemoteSystem(self, remoteSystemAddress,
...
#+end_src
   ...
   that will be interpreted by the local Actor System.  If the
   specified string is not recognizeable by the local Actor System, no
...
** deRegisterRemoteSystem
   ...
   If the remote Actor System is still running and communicating with
   the local Actor System then it will automatically re-register
   itself despite having been de-registered by this method.

   If there is no remote Actor System registered for the specified
   address, or if the address cannot be successfully recognized by the
   local Actor System, this method will simply return with no effect.

#+begin_src python
     self.preRegisterRemoteSystem(self, remoteSystemAddress)
#+end_src

   The ~remoteSystemAddress~ parameter is specified in the same manner
   as for the ~preRegisterRemoteSystem()~ and is usually the same
   value that was originall passed to the latter method.
...
#+END_SRC
    ...
    The ~shutdown()~ ActorSystem method is used to terminate an Actor
    System and all Actors running in that system.  If issued to a
