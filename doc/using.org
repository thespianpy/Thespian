...
#+HTML_HEAD: <link href="thespian.css" rel="stylesheet">
...
#+LATEX_HEADER: \docrev{2.6}{2015 May 25}
...
acknowledgement from an Actor that it has received an Actor.  Actors
...
As noted in the [[*Message%20Bus%20(Pub/Sub)%20Architecture][Message Bus (Pub/Sub) Architecture]], the Actor model is
...
[[*ActorSystem][ActorSystem]], and the known system bases are describe in [[*ActorSystem%20Implementations][ActorSystem
Implementations]].

Once created, the running ActorSystem reference is stored as a
singleton for the current process.  This means that subsequent
...
Actor making the request becomes the "[[*Parent%20Actor][Parent Actor]]" of the newly
...
described in [[*Dead%20Letter%20Handling][Dead Letter Handling]].


*** Parent Actor

    An Actor which creates other Actors automatically becomes the
    "Parent" of the created Actor(s).  In the example in the [[*Creating%20an%20Actor][Creating
...
*** Actor Failure
   ...
   message.  See [[*PoisonMessage%20Returns][PoisonMessage Returns]] for more details.
...
* Guidelines for Actors and their messages
  ...
  - All messages sent by the ActorSystem to an Actor are derived from
    ...
    ActorSystem can deliver to an Actor are:
       ...
       + ActorExitRequest   (see [[*Killing%20an%20Actor][Killing an Actor]])
       + ChildActorExited   (see [[*Parent%20Actor][Parent Actor]])
       + PoisonMessage      (see [[*PoisonMessage%20Returns][PoisonMessage Returns]])
       + ActorSystemConventionUpdate  (see [[*Other%20Actor%20Systems][Other Actor Systems]])
       + WakeupMessage      (see [[*Timer%20Messages][Timer Messages]])
...
** Message Delivery
  ...
  is redirected to [[*Dead%20Letter%20Handling][Dead Letter Handling]].
...
** Non-startable Actors
  ...
  (see [[*Dead%20Letter%20Handling][Dead Letter Handling]]) then it can perform special actions to
...
** Top-Level Actors
  ...
  [[*Parent%20Actor][Parent Actor]]).  Since there is no Parent Actor, there is nowhere to
  ...
  redirected to the Dead Letter Handler (see [[*Dead%20Letter%20Handling][Dead Letter Handling]]).
...
** Named Actors
  ...
  requested Actor, but the Actor System will remember the Actor
  associated with that name andall subsequent Actor creation requests
  with the same name will return the originally created Actor.
  ...
  considered relative to their convenience:
   ...
   1. All Named Actors are similar to [[*Top-Level%20Actors][Top-Level Actors]] in that they
      are managed by the Actor System itself rather than a Parent
...
** Timer Messages
  ...
  be queued causing eventual delivery failure (see [[*Message%20Delivery][Message Delivery]]).
...
** Actor Troupes
   ...
   * All members of the Troupe are local to each other (i.e. exist
     under the same ActorSystem).
...
** ActorSystem Capabilities
  ...
  common capabilities described in the [[*Common%20Capabilities][Common Capabilities]] section.

** Other Actor Systems

  The current ActorSystem can communicate with other ActorSystems to
  coordinate Actor activities as described in [[*Multiple%20Systems][Multiple Systems]].  To
  support this, the ActorSystems communicate on known transport
  addresses to form a "Convention" of ActorSystems.

  When new Actors are created, the [[*ActorSystem%20Capabilities][ActorSystem Capabilities]] of the
...
The 'logging' parameter of the system capabilities passed to the Actor
System (see below) will specify the primary logging configuration for
Thespian and corresponding Actors (regardless of where the logger is
obtained).

The functionality provided by logging in the Thespian environment
possibly includes sending log messages to a central Logging actor.
...
In addition, the Convention Manager Actor System receives all log
...
#+end_src
  ...
  [[*ActorSystem%20Capabilities][ActorSystem Capabilities]].
...
#+end_src
   ...
   the Actor (or [[*External%20Communications][External Communications]] endpoint) sending the
   message.
   ...
   returned to the sender as described in [[*PoisonMessage%20Returns][PoisonMessage Returns]].  Note
...
#+end_src
   ...
   ActorSystem environment (see [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]).  The value
   ...
   The ~globalName~ argument may be specified (as an arbitrary string)
   to create a [[*Named%20Actors][Named Actors]] instance.
   ...
   will be handled as described in the [[*Dead%20Letter%20Handling][Dead Letter Handling]] section.
...
#+begin_src python
    self.send(self, targetAddress, message)
#+end_src
    ...
    sending the message to [[*Dead%20Letter%20Handling][Dead Letter Handling]] if it cannot be
    delivered.

** wakeupAfter

#+begin_src python
    self.wakeupAfter(self, timePeriod)
#+end_src
    ...
    entrypoint (the sender will be the Actor itself).  See [[*Timer%20Messages][Timer
    Messages]] for more information.

** handleDeadLetters

#+begin_src python
    self.handleDeadLetters(self, startHandling=True)
#+end_src
...
#+begin_src python
    self.notifyOnSystemRegistrationChanges(self, startHandling=True)
#+end_src

  This method can be called to register or de-register this Actor for
  handling system registration events from other ActorSystems (see
  [[*Other%20Actor%20Systems][Other Actor Systems]]) via ~ActorSystemConventionUpdate~ messages.

** registerSourceAuthority

#+begin_src python
    self.registerSourceAuthority(self)
#+end_src

   This method registers the current Actor as the Source Authority
   handler for the current Actor System as described in the [[*Security%20Considerations][Security
   Considerations]] section of the [[*loadActorSource][loadActorSource]] Actor System API
...
#+end_src
  ...
  capabilities (see [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]).
...
*** requireCapability decorator
  ...
  and that it's value must be "truthy" (i.e. some expression that
  evaluates as truthful).  The decorator can be repeated multiple
  times for a single Actor and it may be used in conjunction with an
  actual ~actorSystemCapabilityCheck()~ method.
...
#+end_src
   ...
   In the above example, the ~MyActor~ Actor can be instantiated
...
** updateCapability
   ...
   updates; see the [[*ActorSystem%20API][ActorSystem API]] for that version).

#+begin_src python
     self.updateCapability(self, capabilityName, capabilityValue=None)
#+end_src
...
#+begin_src python
      actorSys = ActorSystem(systemBase=None, capabilities=None)
#+end_src

    The ~systemBase~ argument should specify which ActorSystem
    implementation base should be used for this ActorSystem, as
    selected from those systems describe in the [[*ActorSystem%20Implementations][ActorSystem
    ...
    keys and values as describe in the [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]
...
#+end_src
   ...
   environment (see [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]).  The value specified
   ...
   describe in [[*Named%20Actors][Named Actors]], this name allows the Actor to be uniquely
   identified and referenced in a manner additional to the
   ...
   load the ~actorClass~ source and start the Actor.
     ...
     * The source specified by the hash must have been loaded into the
       current Actor System, or previously have been obtained from a
       remote Actor System; this operation will *not* cause a query of
...
#+end_src
   ...
   message to this address will complete the ~ask()~ operation with
   that message.
...
#+begin_src python
    sourceHash = actorSys.loadActorSource(filename)
#+end_src
...
[fn:PEP302] The PEP302 Python extension specification defines the
  ...
  [[*Security%20Considerations][Security Considerations]] section below; when an encrypted zipfile is
  loaded it is first decrypted and/or authorized as described therein.

  If the filename is invalid or specifies a corrupted source file,
  then the returned sourceHash it is not added to the list of
  available sources, but there is no other feedback (excepting perhaps
  the ~/tmp/Thespian.log~ logfile).
...
*** Security Considerations
  ...
  Actor System at any one time.  Any Actor can register itself as the
  ...
  By default, at Actor System startup time, there is no Source
  Authority, thus allowing any ~loadActorSource()~ input to be loaded
  and utilized.  This is convenient for debugging and simple local
  applications, but not recommended for production deployments.
  Typical best practices for production initialization of a Thespian
  environment are to startup the ActorSystem and then immediately load
  a Source Authority that will validate all subsequent sources loaded
  into that system.
...
**** Multi-System Source Authorization
  ...
  The Source Authority registration is unique to an Actor System.
  Different Actor Systems in a [[*Multiple%20Systems][Multiple Systems]] configuration can each
  have a separate Source Authority that may perform different
  authorization techniques.  However, as described in [[*Actor%20Code%20Distribution][Actor Code
  Distribution]], Actor Systems joined together in a Convention can
  exchange sources on demand to satisfy Actor creation requests.
  sources exchanged between Actor Systems in this manner are not
  ...
  capability setting in the [[*Common%20Capabilities][Common Capabilities]] section).
...
** PoisonMessage
   ...
   [[*PoisonMessage%20Returns][PoisonMessage Returns]]).
...
** WakeupMessage
  ...
  call (see [[*Timer%20Messages][Timer Messages]]).
...
** ChildActorExited
   ...
   The Parent of [[*Named%20Actors][Named Actors]] is the ActorSystem itself; no
...
** DeadEnvelope
   ...
   described in [[*Dead%20Letter%20Handling][Dead Letter Handling]].  The ~DeadEnvelope~ is a wrapper
...
** ActorSystemConventionUpdate
   ...
   convention membership (as described in [[*Other%20Actor%20Systems][Other Actor Systems]]).  This
...
** ValidateSource
   ...
   ActorSystem, the Source Authority simply discards this message and
   takes no further action.

   If the specified source is valid for running in the current
   ActorSystem, the Source Authority should return a zipfile
   containing *compiled sources only* to the sender in a
   [[*ValidatedSource][ValidatedSource]] message.
...
** ValidatedSource
   ...
   this response is a zipfile in zipimport format providing the
   *compiled sources only* that were extracted from the input data
   ...
   ActorSystem environment (and any other Actor Systems that are part
   of the Convention).

   Only compiled objects are returned (with ~.pyc~ or ~.pyo~
   extensions) in the returned zipimport file to further preserve
   security: an attacker with visibility into the memory of the
   ActorSystem would need to locate and de-compile the body of the
   zipfile instead of being granted access directly to the decrypted
   original sources.
...
*** Attributes
  ...
  * ~.sourceZip~ -- the zipimport-format decrypted and validated
    compiled form of the originally provided source.
...
** Thespian_StatusReq
   ...
   [[*Thespian_SystemStatusResponse][Thespian_SystemStatusResponse]] or a [[*Thespian_ActorStatusResponse][Thespian_ActorStatusResponse]]
...
** Thespian_SystemStatusResponse
   The ~Thespian_SystemStatusResponse~ object is returned by the
...
transport type: the Actor System provides the message delivery and
underlying Actor support functionality.

  * The implementation of the Actor System can be changed or
...
requests via the ActorSystem object itself.  In addition, the Actor
System must be initially created to provide an environment for all
Actors that are subsequently run within that Actor System; this
creation is the responsibility of code in this external environment.
When external code calls the ~ActorSystem()~ constructor, an Actor
System is started if one is not already running.

An argument may be passed to the [[*ActorSystem][ActorSystem]] constructor to specify
the systemBase (a.k.a Actor System implementation) that should be used
for starting the Actor System if one is not already running; if an
Actor System is already running, this argument is ignored.  If no
argument is specified and no Actor System is running, the default
[[*simpleSystemBase][simpleSystemBase]] Actor System will be started.
...
As described in [[*ActorSystem%20Capabilities][ActorSystem Capabilities]] each ActorSystem has a set of
...
Actor is compatible with the ActorSystem.
...
ActorSystems:
  ...
  * *Python Version* :: This is set to the tuple value of the Python
  ...
  * *Thespian Generation* :: This is the Thespian API functionality
  ...
  * *Thespian Version* :: This is the identifier for the version of
  ...
  * *Thespian ActorSystem Name* :: This is the name of the running
  ...
  * *Thespian ActorSystem Version* :: This is the ActorSystem version
  ...
  * *AllowRemoteActorSources* :: This capability specifies whether or
       ...
       Actor Systems as described in the [[*Actor%20Code%20Distribution][Actor Code Distribution]]
       ...
       current Actor System.  If this capability is not specified, the
       default is *Yes* which will accept sources loaded on any of the
       remote Actor Systems that are part of the Convention.
...
** simpleSystemBase
  ...
  Testing and other very simplistic Actor environments:
    ...
    * No support for [[*Multiple%20Systems][Multiple Systems]] (e.g. Conventions)
  ...
  The simpleSystemBase uses the following [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]:

  * *Thespian ActorSystem Name* :: "simpleSystemBase"
...
in a Convention as described in [[*Multiple%20Systems][Multiple Systems]].

The multiprocQueueBase uses the following [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]:

  * *Thespian ActorSystem Name* :: "multiprocQueueBase"

  * *Admin Address* :: the address for the Admin queue. This is an
...
The multiprocTCPBase uses the following [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]:
...
other systems accessible via UDP/IP.
  ...
  The multiprocUDPBase uses the following [[*ActorSystem%20Capabilities][ActorSystem Capabilities]]:

  * *Thespian ActorSystem Name* :: "multiprocUDPBase"
