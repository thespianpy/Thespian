...
#+LATEX_HEADER: \docrev{2.26}{2025 Feb 03}
...
*** Logging Level Management
    :PROPERTIES:
    :CUSTOM_ID: h:3ef538e8-a989-481b-992d-a9d165a2ce26
    :END:
...
*** Logging Transmit Overruns
    :PROPERTIES:
    :CUSTOM_ID: h:46850001-c8cf-42e6-90cf-dd7236ae9436
    :END:
...
*** Gotchas
  ...
  * If using the ~simpleSystemBase~ (currently the default if no base
    is specified on the initial ~ActorSystem()~ call) then timeouts
...
*** Potential issues
    :PROPERTIES:
    :CUSTOM_ID: h:815365aa-17f0-46a0-a6c9-def03eb7c5e3
    :END:
...
The multiprocTCPBase uses the following [[#hH-a17e6c70-5592-4d06-b818-bd25350c4c53][ActorSystem Capabilities]]:
  ...
  * *Convention Address.IPv4* :: tuple or string or array of tuples/strings
...
:PROPERTIES:
:ID:       4652776f-0aba-4564-8f50-2add516f6508
:CUSTOM_ID: h:ca576db1-3923-4db4-aaad-e6c41730f552
:END:
  ...
  variables must be set *before* the ActorSystem is started for them to affect
  the ActorSystem and any Actors started within that ActorSystem.

  * ~THESPIAN_BASE_IPADDR~ :: specifies the local IP address to use for Actors on
    ...
    * default :: locally probed address

  * ~THESPIAN_MAX_PENDING_TRANSMITS~ :: Specifies the number of transmit messages
    ...
    prevents overflowing the system's ~socket.send()~ limitations.
  ...
  * ~THESPIAN_MAX_QUEUED_TRANSMITS~ :: Actors can transmit messages faster than
    ...
    Actor.  Normally, the ~.send()~ method is asynchronous, queueing the message
    for transmit and returning immediately to the Actor's code, but when this
    theshold is reached, a ~.send()~ will block (implementing "backoff" or "back
    pressure"). This environment variable overrides the default threshold value.

    If this value is changed, the ~THESPIAN_QUEUED_TRANSMIT_UNBLOCK_THRESHOLD~
  ...
  * ~THESPIAN_QUEUED_TRANSMIT_UNBLOCK_THRESHOLD~ :: When an Actor's ~.send()~
    method is blocked due to reaching the THESPIAN_MAX_QUEUED_TRANSMITS threshold
    (either the explicit or default threshold), the ~.send()~ will not unblock
    ...
    the ~THESPIAN_QUEUED_TRANSMIT_UNBLOCK_THRESHOLD~ environment variable.

    This value *should always be less* than the ~THESPIAN_MAX_QUEUED_TRANSMITS~
    value.
  ...
  * ~THESPIAN_DROP_TRANSMITS_LEVEL~ :: It is possible to configure the Actor to
    drop transmit requests if there are too many queued.  Message queueing can
    occur via the Actor's intentional ~.send()~ call, or implicitly via Actor
    ...
    If this value is set lower than the ~THESPIAN_MAX_QUEUED_TRANSMITS~ level
    then the ~.send()~ method will never block (exerting back-pressure) but
    ...
    * default :: 100 plus the maximum queued transmits threshold (which can be
      set via ~THESPIAN_MAX_QUEUED_TRANSMITS~).

  In addition to the above, the logging environment variables described in
  [[#hH-856a7ffe-676b-42a9-95eb-bd89a5810f53][Thespian Internals Logging]] may be set.
...
The multiprocUDPBase uses the following [[#hH-a17e6c70-5592-4d06-b818-bd25350c4c53][ActorSystem Capabilities]]:
  ...
  * *Convention Address.IPv4* :: tuple or string or list of tuples/strings
...
* Potential Concerns and FAQ
  :PROPERTIES:
  :CUSTOM_ID: h:f3cb3837-435b-4551-91bc-1913af405a67
  :END:

 Some items of potential concern when working with Thespian and actor
 systems in general:

 * Dropped messages ::  While Thespian does its best to ensure that
 ...
 * Message size limitations :: Thespian itself does not impose message
 ...
 * Use of Thespian in conjunction with some other scheduling system
   ...
   system), this is usually incompatible with any *other* scheduling system
   ...
   frameworks that also expecting to manage that context.
