...
#+LATEX_HEADER: \docrev{4.0.1}{2025 Mar 11}
...
** 2025
 ...
 |---------+------------+--------------------------------------------------------------------------------------------+---------------------------------------------+---|
 |   4.0.1 | 2025-03-11 | Fix  for [[https://github.com/thespianpy/Thespian/issues/86][Issue #86] (local IP address) |                                             |   |
 |   4.0.0 | 2025-02-03 | Better fix for [[https://github.com/thespianpy/Thespian/issues/70][Issue #70]] (fixes Windows)                                                   | Use ~selectors~ for socket handling.        |   |
...
** 2023
 ...
 | Release |       Date | Significant Bugfixes                                          | Significant Features |
 |---------+------------+---------------------------------------------------------------+----------------------|
 |  3.10.7 | 2023-01-15 | Fix [[https://github.com/thespianpy/Thespian/issues/70][Issue #70]]: setting ~TCP_NODELAY~ on MacOS sometimes fails |                      |
 |         |            |                                                               |                      |
...
** 2021
 ...
 | Release |       Date | Significant Bugfixes                                       | Significant Features |
 |---------+------------+------------------------------------------------------------+----------------------|
 |  3.10.5 | 2021-05-26 | Admin forwarding; [[https://github.com/thespianpy/Thespian/issues/75][Issue #75]]: idempotent globalName creates |                      |
 |  3.10.4 | 2021-05-12 | Fix [[https://github.com/thespianpy/Thespian/issues/62][Issue #62]]: logDefs no ~get~ for ~None~                 |                      |
...
:PROPERTIES:
:ID:       a62165bc-c0c5-402a-9649-9f16d6ea345f
:END:
    ...
    * Adds ~ENOTCONN~ as a send in-progress status for backoff.  Contributed by
      Grzegorz Banasiak.
    * Longer timeouts in test_HAConvention.py and test_manyActorSystems.py to
      support Windows. Contributed by Grzegorz Banasiak.
    * Adds examples/socketstress.py as another means to perform stress/load
      testing.  Contributed by Grzegorz Banasiak.
    * Updates to test_runcommand.py to support Windows and skip test_actorSignals
      on the same platform since it does not support POSIX signals. Contributed
      by Grzegorz Banasiak.
    * Added support for ~THESPIAN_MAX_PENDING_TRANSMITS~,
      ...
      environment variables on ~ActorSystem~ startup. See [[file:using.org::#h:ca576db1-3923-4db4-aaad-e6c41730f552][Configuration
      Environment Variables]] in "Using Thespian" for details.
    * Updates to ~diagnose.py~ for better Windows support and additional TCP
      socket information.
...
*** 3.10.7 <2023-01-15 Sun>
    ...
    * Fixes issue 70 where spurious connections would not allow setting
      ~TCP_NODELAY~ on the incoming socket (probably due to non-TCP connection
      attempt or Ipv6 alternate headers).

*** 3.10.6 <2022-01-09 Sun>

    * Adds Beta support for convention fault tolerance by allowing the
      ...
      Contributed by Arnab Chanda.

*** 3.10.5 <2021-05-26 Wed>

    * Simultaneous create actor requests for a globalName actor could
      result in duplicates if processing overlapped in the Admin.
      This fixes them to ensure they are idempotent. Fixes
      [[https://github.com/kquick/Thespian/issues/75][Issue #75]].

    * Fixed an issue in Admin forwarding where message forwarding
      could suffer significant delays in the absense of any other
      traffic.

    * Updates to the documentation of the Thespian Director, as noted in
      [[https://github.com/kquick/Thespian/issues/77][Issue #77]].  Contributed by htarnacki.

    * Additional documentation of potential issues with various system
      bases in the Using Thespian documentation.

*** 3.10.4 <2021-05-12 Wed>

    * Improvements to logging configuration to properly determine the
      lowest logging level and set the proper logging level.  Fixes
      [[https://github.com/kquick/Thespian/issues/70][Issue #70]].  Contributed by Todd Cook.

    * Fixed [[https://github.com/thespianpy/Thespian/issues/62][Issue #62]]: initialization without logging could get an
      exception ~AttributeError: 'NoneType' object has not attribute
      'get'.  Fix contributed by Todd Cook.

    * Misc. fixes for missing imports and update multiproc daemon
      attribute handling for Python 3 version of the multiprocessing
      library.  Contributed by Todd Cook.

    * Fixed reference to Windows default path for ~thesplogd.cfg~ in
      Thespian Director documentation.  Contributed by Todd Cook.

    * Various wording fixes and clarification in the documentation.
      Contributed by Hannes Lerchl.
