#+TITLE: [[http://thespianpy.com][@@html:<img src="thesplogo2.png" alt="Thespian" width="50%" display="inline" />@@]] In-Depth Introduction
...
statements (except for the first) in the output above because the
default logging configuration is to write to the console.
...
def run_example(systembase=None):
    hello = ActorSystem(systembase, logDefs=logcfg).createActor(Hello)
    goodbye = ActorSystem().createActor(Goodbye)
    greeting = ActorSystem().ask(hello, 'are you there?', 1.5)
