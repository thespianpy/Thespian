...
class Command(object):
    """Defines the Command to be run by the RunCommand Actor.
       ...
       specifies that no logging is to be performed, otherwise it can
       be an object that provides a ".error" and a ".info" method.
...
class CommandOutput(object):
    """Message specifying (possibly partial) output received from the
       command, and sent to the output_updates Actor.  This may be sent
       multiple times as output is generated by the running process.
    """
...
class RunCommand(ActorTypeDispatcher):
    ...
    def _set_command_timeout(self, command):
        if command.timeout:
            td = command.timeout
            command.expiration = datetime.now() + td
            self.wakeupAfter(td, payload=self.command_num)
    ...
    def _start_command(self):
        ...
        getattr(command.logger or logging, "info")(command.logtag + " CMD: " + logcmd)
        ...
        if command.timeout:
            end_time = self.start_time + command.timeout
        ...
        self._add_output(command, 'normal', out)
        self._add_output(command, 'error', err)
    ...
    def _log_finished_command(self, result):
        ...
        logger = result.command.logger or logging
        lognormal = getattr(logger, 'info')
        logerror = getattr(logger, 'info' if result.command.error_ok else 'error')
        for each in lno:
    ...
    def receiveMsg_WakeupMessage(self, wakemsg, sender):
        ...
        if getattr(self, 'tried_terminate', False):
            ...
            if subp.poll() is None:
                getattr(command.logger or logging, "error")(
